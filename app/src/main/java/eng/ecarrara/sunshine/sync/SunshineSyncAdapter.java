...
import eng.ecarrara.sunshine.MainActivity;
...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        ...
        String[] periodFormattedForecast = null;
        ...
        try {
            ...
            Uri builtUri = Uri.parse(FORECAST_BASE_URL).buildUpon()
                    ...
                    .appendQueryParameter(DAYS_PARAM, Integer.toString(period))
                    .build();
        ...
        try {
            getWeatherDataFromJson(forecastJsonStr, period, location);
    ...
    private void getWeatherDataFromJson(String forecastJsonStr, int numDays,
        ...
        for(int i = 0; i < weatherArray.length(); i++) {
            ...
            weatherValues.put(WeatherContract.WeatherEntry.COLUMN_LONG_DESC, description);
        ...
        if(cVVector.size() > 0) {
            ...
            String yesterday = WeatherContract.getDbDateString(calendar.getTime());
    ...
    private long addLocation(String locationSetting, String cityName, double lat, double lon) {
        Log.d(LOG_TAG, "Inserting " + cityName + " with coord: " + lat + " " + lon);
        long returnId = -1L;
        ...
        if (cursor.moveToFirst()) {
            Log.d(LOG_TAG, "Location already exists.");
            int locationIndex = cursor.getColumnIndex(WeatherContract.LocationEntry._ID);
            returnId = cursor.getLong(locationIndex);
        } else {
            Log.d(LOG_TAG, "Location not found inserting.");
            ...
            Uri insertedItem =
                    getContext().getContentResolver().insert(WeatherContract.LocationEntry.CONTENT_URI, values);
            returnId =  ContentUris.parseId(insertedItem);
        }
